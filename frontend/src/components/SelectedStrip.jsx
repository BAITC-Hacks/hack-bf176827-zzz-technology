...
export default function SelectedStrip({card, status, onSelect, onEmphasis}) {
  ...
  return <div className="strip">
    ...
    <div className="strip__block strip__block--hover" onMouseEnter={() => onEmphasis('in')} onMouseLeave={() => onEmphasis(null)} title="Наведите: входящие переводы поверх остальных">
      <span className="strip__label"><i className="swatch swatch--brand" />Получено</span><span className="strip__value">{fmtKzt(st.inK)}</span><span className="strip__sub">{inSub}</span></div>
    <div className="strip__block strip__block--hover" onMouseEnter={() => onEmphasis('out')} onMouseLeave={() => onEmphasis(null)} title="Наведите: исходящие переводы поверх остальных">
      <span className="strip__label"><i className="swatch swatch--ink" />Отправлено</span><span className="strip__value">{fmtKzt(st.outK)}</span><span className="strip__sub">{outSub}</span></div>
    ...
    {st.biggest && <div className="strip__block"><span className="strip__label">Крупнейший перевод</span><span className="strip__value">{fmtKzt(st.biggest.e.sum_kzt)}</span>
      <span className="strip__sub strip__sub--gid">{st.biggest.dir} <button className="link" onClick={() => onSelect(st.biggest.other)} title="Открыть контрагента">{st.biggest.other}</button></span></div>}
