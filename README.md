...
Для каждой роли считается скор 0–1 по правилу с порогом; роль узла — с наибольшим скором,
`role_score` — этот скор. Если ни одно правило не набрало 0.5, узел — `peripheral`.

| Роль | Условие | Скор | Кол-во |
|---|---|---|---|
| `consolidator` | плательщиков ≥ 3 | `min(1, in_deg/6) × (1 − min(1, pass_through))`, +0.15 если платят ≥ 2 seed; у обрезанных обходом ×0.7 | 31 |
| `coordinator` | вход ≥ 2, выход ≥ 2, betweenness ≥ P97, плательщики из ≥ 2 кластеров или ≥ 3 seed выше | `0.5 + 0.5·(0.5·btw/P99 + 0.25·кластеров/3 + 0.25·seed/5)` | 43 |
| `distributor` | получателей ≥ 5 и ≥ 2·плательщиков | `min(1, out_deg/30)`, ×0.6 если плательщиков ≥ 3 | 16 |
| `transit` | не seed, есть вход и выход, `0.7 ≤ pass_through ≤ 1.3` | `0.5 + 0.5·(1 − |pass_through − 1|/0.3) + 0.2·fast_forward_share` | 105 |
| `terminal` | нет исходящих, узел ≤ 3-го колена (обход шёл дальше), плательщиков ≥ 2 или сумма ≥ 100 000 | `0.5·min(1, in_kzt/500 000) + 0.5·min(1, in_deg/3)` | 153 |
| `peripheral` | иначе | `1 − max(остальных)` | 1 900 |

Узлы 4-го колена без исходящих (`truncated`, 444 шт.) никогда не получают `terminal`: их исходящие не видны,
а не отсутствуют. Приоритет: `0.25·P(in_kzt) + 0.20·P(in_deg) + 0.15·P(seed выше) + 0.15·P(pagerank) + 0.10·P(betweenness) + 0.15·бонус роли`,
где `P` — перцентиль по сети; seed ×0.5, обрезанные ×0.7. Evidence у каждого узла — фраза ≤ 200 символов с числами по шаблону роли.
...
- Детекция аномалий ограничена правилом дробления сумм (≥ 5 входящих, ≥ 60 %
  из них до 15 000 KZT); переводы ниже порога 5 000 в выгрузке не видны.
...
Что изменится в подходе, если сеть вырастет с 2 248 до ~1 млн узлов (реализация не требуется ТЗ):

- **Хранение и загрузка.** Parquet целиком в памяти уже не подойдёт: колоночное хранилище или графовая БД
  (Neo4j GDS, TigerGraph) либо Spark GraphFrames; в памяти Go 1 млн узлов и ~5 млн рёбер ещё помещаются
  (сотни МБ), но пересчёт всех метрик перестаёт быть секундами.
- **Метрики.** Степени, суммы и PageRank линейны и остаются. Betweenness O(V·E) заменяется аппроксимацией
  по сэмплу источников (Brandes–Pich) или локальным k-hop вариантом. Циклы — только длиной 3–4 и только
  вокруг кандидатов из топа.
- **Кластеры.** Louvain → Leiden (быстрее и стабильнее), расчёт по компонентам параллельно.
- **Инкрементальность.** Новые выгрузки пересчитывают только затронутые компоненты; метрики узлов
  хранятся в таблице с версиями, детерминизм сохраняется за счёт фиксированного порядка обхода.
- **Интерфейс.** Полный граф не рисуется: сервер отдаёт ego-подграфы и агрегаты по кластерам,
  раскладка считается на сервере, поиск по gid через индекс.
- **LLM-слой.** Инструменты ассистента те же, меняется источник: запросы к БД вместо памяти; кэш ответов
  переезжает в хранилище с TTL.
